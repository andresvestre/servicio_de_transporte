...
export class ContextTransport implements IContextTransport {
  ...
  async executeQuery<TEntity> (query: string, parameters?: ParametersDB |
    undefined): Promise<TEntity[] | undefined> {
    await this.connect()
  ...
  async executeQueryScalar<TEntity> (query: string, parameters?:
    ParametersDB | undefined): Promise<TEntity | undefined> {
